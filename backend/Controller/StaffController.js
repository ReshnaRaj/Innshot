...
module.exports.addresort = async (req, res, next) => {
  try {
    ...
    // req.body the data name is what is write inside the formdata variable that variable name
    ...
    console.log(savedResort, "resort addedd new  ");
...
module.exports.posteditresort = async (req, res, next) => {
  try {
    console.log(req.body, "request  editing working.... properly....");
    ...
    let { resortname, place, number_room, address, description, price, phone ,service} =
      req.body;
      // console.log(id,"how i getting id")
    // let {image,document}=req.files
    // let resortId=req.params.id
    // above code the first name is we already give in frontend that name it should be same  in while updating 
    // the below code first is from model and second name is from formdata variable name
    ...
    // console.log(resort,"resort updated sooooo") this is old data consoling
...
module.exports.posteditadv = async (req, res, next) => {
  try {
    ...
    const { advact, advprc, advtime, advplace, advresort, advdescription } =
      req.body;
    ...
    const adventure = await AdvModal.findByIdAndUpdate(
      { _id: id },
      {
        activity: advact,
        place: advplace,
    ...
    res.json({
      result: adventure,
      message: "Adeventure Updated Successfully",
      success: true,
    });
  } catch (error) {
    console.log(error);
...
module.exports.addDestination = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ newdest, message: "New Destination Added", created: true });
  } catch (error) {}
};
...
module.exports.editdestination=async(req,res)=>{
  try {
    console.log("edit resort datas....")
    const id=req.params.id
    console.log(id,"oooooo")
    console.log(req.body,"destination datass")
    ...
    const destination=await DestinationModel.findByIdAndUpdate(
      ...
      res.json({result:destination,message:"destination Updated Success",success:true})
  } catch (error) {
